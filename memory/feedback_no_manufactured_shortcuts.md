...
When the Go model already prescribes how a change should land, do not present "smaller / TS-local / defer" variants alongside the model-correct path as if they were peer options. Picking the model-correct path is not "more work" — it is the work.

**Why:** During pulse-transport Phase 4, I presented three options for latency-live on drag: (A) TS-local recompute, (B) TS→Go node-move, (C) defer. The model already says Go owns transport timing — B was the only path consistent with the model; A reintroduced duplicated formulas (the exact drift Phase 3 just removed), and C left the feature half-built. The user pushed back: "why are partial work items being promoted as the first option? I'm more confused about a fast typist suggesting it does less typing." Subagent execution cost is not a constraint worth trading model coherence against.

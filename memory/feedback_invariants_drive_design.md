...
Related: [[feedback_no_manufactured_shortcuts]] (don't offer partial paths when the model prescribes the full one); [[feedback_derive_model_from_visual_spec]] (derive the implied model up front; don't accept patches that preserve a wrong model); [[feedback_go_vs_coordinator_bias]] (name the contract violated, not the symptom).

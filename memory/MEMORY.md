...
- [feedback_reflect_dont_create_store.md](feedback_reflect_dont_create_store.md) — "Don't use a store" = don't create a NEW TS store for a streamed bit; reusing an existing store to reflect a Go-owned value is fine (scene-tori toggle is the pattern)

...
Full design lived in the branch-local doc `docs/planning/visual-editor/3d-editor.md` (branch `task/editor-3d-plan`); it was stripped at merge per CLAUDE.md's branch-local-docs rule and no longer exists — hence this memory as the durable record. Related: [[feedback_code_self_defends]] (durable guard is structural code, not a doc/memory — encode the design-layer / device-fallback-layer split so implementing OrbitControls would violate a module boundary), [[feedback_go_vs_coordinator_bias]].

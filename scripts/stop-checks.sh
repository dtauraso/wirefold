...
for chk in check-trace-kind-parity check-no-ts-timers check-message-kind-parity check-edit-op-parity check-bridge-literal-parity check-slot-phase-boundary check-generated check-wire-prop-used check-no-camera-roundtrip check-polar-only-nav check-no-await-on-bridge check-ts-computes-no-geometry check-ts-shading-from-go check-send-rule-parity check-gofmt check-buffer-layout-parity; do

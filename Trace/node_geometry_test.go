...
func TestNodeGeometryEmitsEvent(t *testing.T) {
	tr := New(8)
	tr.NodeGeometry("N1", 1, 2, 3, []PortGeom{
	...
	if e.Node != "N1" || e.NX != 1 || e.NY != 2 || e.NZ != 3 {
		t.Fatalf("node/center mismatch: %q (%v,%v,%v)", e.Node, e.NX, e.NY, e.NZ)
	}
	if len(e.Ports) != 2 {
	...
	if got.Kind != "node-geometry" || got.Node != "N1" || got.NX != 1 || got.NY != 2 || got.NZ != 3 {

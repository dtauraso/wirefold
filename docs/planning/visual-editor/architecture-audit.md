...
4. **edgeSeeds was a fossil superseded by bootstrap nodes — ✅ RESOLVED (commits `4fed3df3` code, `b8787f5e` memory).** History settled it: edge `data.seed`/`edgeSeeds` → node initialSlots (`803873c3`, 2026-05-19) → replaced by dedicated **bootstrap Input nodes** (`8a8dd842`, 2026-05-19: kind `Input`, `data.init=[seed]`, `repeat=false`, single-firing the seed into the receiving port at tick-0 via a real edge) → inert Go path deleted (`d87cb332`, 2026-05-22). What remained was a TS-only fossil — parsed/validated/serialized but set by nothing (no topology, no UI), ignored by Go. Removed the entire TS pipeline (types, parse, validate, serialize, and the `requiredInputDiagnostics` edgeSeeds branch — behavior unchanged since bootstrap feeds the port via a real edge). Corrected the stale `feedback_edge_seed_required_for_rings` memory to describe the bootstrap mechanism.
...
### Status
All Organization-Health findings (#1–#7) AND all Architecture findings (#1–#6) resolved (see ✅ tags above). edgeSeeds (#4) turned out to be a fossil already replaced by bootstrap nodes — removed.

Note: a pre-existing Go test `Trace.TestMarshalEventMatchesFixture` and ~10 pre-existing TS failures (`diff-core`, `fold`, two `parseSpec` cases) are unrelated to this audit and remain open.

...
- Two HTML specs under `docs/`: `docs/clock-dialog/index.html` and `docs/go-authoritative-clock/index.html`. The latter is now a 12-tab spec: Overview, Division, **Goroutines**, The Bridge, Clock Change, Contract, Play/Pause, TS → Go, Doc Updates, Plan, Phases, Verify.
- This session added the **Goroutines** tab (after Division): a simple table of the actual goroutines — main, node loop (one per node, the real work), stdin reader (one, always reading even while paused), trace drain (one), short-lived per-send helpers — each "Its job" cell enumerated as ordered steps. Lead framing: the concurrency IS the nodes; there is no global clock/scheduler goroutine, and `PacedWire` has no goroutine of its own (driven by sender/receiver loops via lock + cond var; slot/backpressure live in the destination node). A note ties it to the clock change: in the target model each node loop also wakes ~16ms to emit bead positions and self-deliver, replacing today's wait on the TS "delivered" message.

...
- **Active branch:** none. `main` is current; last work merged was validation-flag feature removal + dead-code/lint cleanup.
...
2. **Dead code + lint cleanup.** Deleted unused `AppendMultiPaced` method (`nodes/Wiring/builders.go`); modernized two `if n<1` clamps to `max(samples,1)` in `nodes/Wiring/curve_params.go`.

...
3. **Pre-existing test failures (predate audit, unrelated):** TS — `parseSpec.test.ts` (2: legacy `timing.steps` not dropped; legend bad-kind not rejected), `fold.test.ts` (1: "expanded fold emits a frame and keeps members + edges intact" — `specToFlow` returns `[]` for member nodes when fold is expanded; the suite now loads correctly after import was repointed from deleted `src/webview/rf/adapter` to `src/webview/state/adapter/spec-to-flow`). Go — `Trace.TestMarshalEventMatchesFixture`. (The two contract failures — topology-edge-handles, trace-event-fields — were FIXED this session.)

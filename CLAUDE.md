...
1. Add an entry to `NODE_DEFS` in `tools/topology-vscode/src/schema/node-defs.ts`
   ...
   PascalCase Go kind name verbatim). Node rendering is generic — `three/NodeInstances.tsx`
...
NOT feed the render path: `EdgeTube` (`three/EdgeTube.tsx`, the edge renderer) reads only
...
All nodes are rendered generically by `three/NodeInstances.tsx` from the buffer's Node

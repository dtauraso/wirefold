...
1. Add an entry to `NODE_DEFS` in `tools/topology-vscode/src/schema/node-defs.ts`
   ...
   PascalCase Go kind name verbatim). Node rendering is generic — `scene-graph.tsx`
...
`tools/topology-vscode/src/webview/three/scene-graph.tsx`. A new wire prop must be
...
All nodes are rendered generically by `scene-graph.tsx` `GraphNode` (sphere mesh +

...
overlays) into a **binary content buffer** and streams it. `buffer-scene.tsx` decodes
...
1. Add an entry to `NODE_DEFS` in `tools/topology-vscode/src/schema/node-defs.ts`
   ...
   PascalCase Go kind name verbatim). Node rendering is generic — `buffer-scene.tsx`
...
**Bridge surface:** **Go → TS** is the binary content buffer (`buffer-snapshot`) plus the
`node-label` id/label/kind sidecar — Go reporting the whole scene. **TS → Go** is
`raw-input` (raw pointer/wheel + raycast hit → Go's gesture FSM), spec save/load, and a
single geometry-CRUD `edit` message with **exactly three ops** (create / update / delete)
plus the play/pause control signal (see `nodes/Wiring/stdin_reader.go` `applyEdit` and
...
All nodes are rendered generically by `buffer-scene.tsx` from the buffer's Node block
(sphere mesh + border ring), keyed off `node.data.fill`/`node.data.stroke` from

...
func (g *Node) Update(ctx context.Context) {
	...
	for {
		...
		// NOT the left input number (1→0, 0→1) as it is captured, so the held/displayed
		// left value and the gate use the inverted input. The right input is captured as-is.
		if v, got := drainLatestReal(g.FromLeft); got {
			nv := 1 - v
			if !g.HasLeft || g.Left != nv {
				g.Left = nv
				g.HasLeft = true
				emitInputs()
			}
		...
		if g.HasLeft && g.HasRight {
			...
			if now()-dwellStart >= fireDwellMs*time.Millisecond {
				// AND gate over the held values; the left input was NOT'd on capture
				// (1→0, 0→1), so this fires 1 iff left-input==0 AND right==1.
				result := 0

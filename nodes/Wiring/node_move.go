...
// applyNodeMove is the SYNCHRONOUS test/in-process façade over the decentralized
// path: it mail-sorts the move to the moved node's inbox and every incident edge's
// inbox (exactly what the TS sender + stdin reader do live), then blocks until each
// mover has handled it. This lets a test assert the recompute result deterministically
// without a central coordinator. No-op for an unknown node.
//
// It requires the movers' goroutines to be running (Start). For tests that do not
// Start, it falls back to invoking each mover's handle directly.
func (md *MoveDispatch) applyNodeMove(nodeID string, x, y, z float64) {
	nm := md.nodeMovers[nodeID]
	if nm == nil {
		return
	}
	// Collect the node's own key plus every incident edge key.
	var keys []string
	...
	// No goroutines running (unit test): invoke handlers directly.
	msg := moveMsg{NodeID: nodeID, X: x, Y: y, Z: z}
	nm.handle(msg)
	for _, em := range md.edgeMovers {
		if em.srcID == nodeID || em.dstID == nodeID {
			em.handle(msg)

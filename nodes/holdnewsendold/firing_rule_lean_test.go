...
func TestFireOnReceiveLean(t *testing.T) {
	...
	node := &Node{
		...
		FromPrevHoldNewSendOldNode: Wiring.NewInPaced(inPw, ctx, "in", "FromPrevHoldNewSendOldNode", tr, clk),
	...
	obs0 := Wiring.NewInPaced(outPw0, ctx, "obs0", "In", tr, clk)
	obs1 := Wiring.NewInPaced(outPw1, ctx, "obs1", "In", tr, clk)

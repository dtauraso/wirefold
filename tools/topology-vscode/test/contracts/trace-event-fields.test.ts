...
describe("trace-event-fields contract", () => {
  ...
  it("fixture has one event for each kind variant", () => {
    ...
    expect(kinds).toEqual(new Set(["recv", "fire", "send", "done", "edge-bead", "geometry", "pulse-cancelled", "node-geometry", "arrive", "node-bead", "camera", "scene-tori", "scene-poles", "node-poles", "angle-labels", "sel-sphere-poles", "handholds", "labels-global", "badges-global", "overlays-vis", "double-links", "node-status"]));
  });

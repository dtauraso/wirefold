...
function simpleStyle(bg: string, i: number, n: number): CSSProperties {
  const s: CSSProperties = { background: bg }; if (n > 1) s.top = `${((i + 1) / (n + 1)) * 100}%`; return s;
}
...
export function GenericNode({ id: nodeId, type, data }: NodeProps<NodeData>) {
  ...
  const container: CSSProperties = { background: def.bg, border: `1px solid ${def.border}`, borderRadius: 4, padding: "4px 8px", minWidth: def.minWidth ?? 70, fontSize: 11, color: def.text, boxShadow: flashing ? `0 0 8px 2px ${def.accent}` : undefined };
  return (
...
function renderDefHandles(def: NodeDef) {
  ...
  return [...targets.map((p, i) => <Handle key={p.id} type="target" position={Position.Left} id={p.id} style={simpleStyle(p.accent ?? def.accent, i, targets.length)} />), ...sources.map((p, i) => <Handle key={p.id} type="source" position={Position.Right} id={p.id} style={simpleStyle(p.accent ?? def.accent, i, sources.length)} />)];
}
...
function renderSnapDots(drag: ActiveDrag | null) {
  ...
  return (["left","right","top","bottom"] as Side[]).flatMap((side) =>
    ([0,1,2] as const).map((slot) => {
      const pct = SLOT_PCT[slot]; const isNearest = drag.nearestSide === side && drag.nearestSlot === slot; const iv = side === "left" || side === "right";
      const dotStyle: CSSProperties = { position: "absolute", width: 6, height: 6, borderRadius: "50%", background: isNearest ? "rgba(80,160,255,0.9)" : "rgba(120,120,200,0.35)", pointerEvents: "none", ...(iv ? { [side]: -3, top: `${pct}%`, transform: "translate(-50%,-50%)" } : side === "top" ? { top: -3, left: `${pct}%`, transform: "translate(-50%,-50%)" } : { bottom: -3, left: `${pct}%`, transform: "translate(-50%,50%)" }) };
      return <div key={`snap-${side}-${slot}`} style={dotStyle} />;
    })
  );
}

function renderDisplay(kind: DisplayKind, data: NodeData): ReactNode {
  if (kind === "queue") {

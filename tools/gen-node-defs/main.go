...
func main() {
	...
	traceKindsPath := filepath.Join(repoRoot, "tools", "topology-vscode", "src", "webview", "rf", "trace-kinds.ts")
	...
	topologyShapePath := filepath.Join(repoRoot, "tools", "topology-vscode", "src", "webview", "rf", "nodes", "topology-shape.ts")
	if err := writeTopologyShape(topologyShapePath, kinds); err != nil {
		fatalf("write %s: %v", topologyShapePath, err)
	}
	fmt.Fprintf(os.Stderr, "gen-node-defs: wrote %s\n", topologyShapePath)
}
...
// writeTopologyShape emits topology-shape.ts — the Go-owned keys that
// nodes/Wiring/loader.go reads from topology.json.
//
// Boundary: view-only fields that Go ignores (position, edge midpointOffset,
// any UI labels not wired into loader.go) are NOT in this file. Those are
// validated/typed by hand in TS. Add a field here only when Go's loader reads it.
func writeTopologyShape(outPath string, kinds []kindEntry) error {
	var buf bytes.Buffer
	w := bufio.NewWriter(&buf)

	fmt.Fprintln(w, `// GENERATED by tools/gen-node-defs — do not edit.`)
	fmt.Fprintln(w, `// Source: nodes/Wiring/loader.go struct tags + nodes/<Kind>/<Kind>.go.`)
	fmt.Fprintln(w, `//`)
	fmt.Fprintln(w, `// This module describes the keys that Go's loader reads from topology.json.`)
	fmt.Fprintln(w, `// View-only fields (position, edge midpointOffset, UI labels not in loader.go)`)
	fmt.Fprintln(w, `// are intentionally absent — they are validated by hand in TS.`)
	fmt.Fprintln(w)
	fmt.Fprintln(w, `// Top-level node fields read by the Go loader.`)
	fmt.Fprintln(w, `export interface TopoNode {`)
	fmt.Fprintln(w, `  id: string;`)
	fmt.Fprintln(w, `  type: string;          // PascalCase Go kind`)
	fmt.Fprintln(w, `  data?: Record<string, unknown>; // per-kind data blob (loader reads wire:"data.*" fields)`)
	fmt.Fprintln(w, `  edgeSeeds?: Record<string, number>; // pre-sent values for ring topologies`)
	fmt.Fprintln(w, `}`)
	fmt.Fprintln(w)
	fmt.Fprintln(w, `// Top-level edge fields read by the Go loader.`)
	fmt.Fprintln(w, `export interface TopoEdge {`)
	fmt.Fprintln(w, `  id: string;`)
	fmt.Fprintln(w, `  source: string;`)
	fmt.Fprintln(w, `  target: string;`)
	fmt.Fprintln(w, `  sourceHandle: string;`)
	fmt.Fprintln(w, `  targetHandle: string;`)
	fmt.Fprintln(w, `  label: string;         // channel name in the substrate`)
	fmt.Fprintln(w, `}`)
	fmt.Fprintln(w)
	// Emit per-kind edgeSeeds key names (input port names) so TS can validate them.
	fmt.Fprintln(w, `// Valid edgeSeeds keys per Go kind (input port names).`)
	fmt.Fprintln(w, `// Derived from *Wiring.In fields — same source as REQUIRED_INPUTS in node-defs.ts.`)
	fmt.Fprintln(w, `export const EDGE_SEED_PORTS: Record<string, string[]> = {`)
	for _, e := range kinds {
		ins := filterPorts(e.ports, "in")
		if len(ins) == 0 {
			continue
